...
top_10_aeroportos = df['source airport'].value_counts().head(10).index.tolist()
...
custos_minimos = df_top10.groupby(['source airport', 'destination airport'])['custo'].min()

# 6. CRIAR A MATRIZ DE CUSTOS VAZIA (10x10)
matriz_custos = pd.DataFrame(
    np.inf, 
    index=top_10_aeroportos, 
    columns=top_10_aeroportos
)
